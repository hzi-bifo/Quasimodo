...
# -*- coding: utf-8 -*-
'''
File: vicuna_config_generator.py
Created Date: March 6th 2020
Author: ZL Deng <dawnmsg(at)gmail.com>
---------------------------------------
Last Modified: 6th March 2020 12:20:53 pm
'''
...
@click.command()
@click.argument('fq_dir', type=click.Path(exists=True))
@click.argument('out_dir', type=str)
def config_generator(fq_dir, out_dir):
    (fq_dir, out_dir) = list(map(path.abspath, [fq_dir, out_dir]))
    if not fq_dir.endswith('/'):
        fq_dir += '/'
...
// [default -1]
LibSizeLowerBound	150
// [default -1]
LibSizeUpperBound	1000
...
outputDIR {out_dir}
'''.format(fq_dir=fq_dir, out_dir=out_dir)
    print(out_config)
