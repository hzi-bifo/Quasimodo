...
```shell
Rscript --vanilla scripts/assembly_radarplot.R -f <the assembly_metaquast_ranking.tsv file produced by the pipeline QuasiMode> [-o the output file name for the radar plot]
```

```shell
Options:
	-f CHARACTER, --file=CHARACTER (required)
		assembly_metaquast_ranking.tsv file generated by QuasiModo
